...
namespace Blockade_Defense
{
    class SuperTower : Tower
    {
        ...
        public SuperTower(int x, int y, int atkPwr, int atkRad, int atkFreq, int cst, Texture2D text, Texture2D atkText)
            : base(x, y)
        {
            attackPower = atkPwr;
            attackRadius = atkRad;
            ...
            radiusRec.X = (TowerRec.X - 240);
            radiusRec.Y = (TowerRec.Y - 240);
            radiusRec.Width = 560;
            radiusRec.Height = 560;
            cost = cst;
            coolDown = 0;
            isAttacking = false;
            attackTracker1 = 0;
